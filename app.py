...
def load_data():
    ...
    try:
        ...
        # # Convert all values to numeric and replace invalid ones
        # df = df.apply(pd.to_numeric, errors='coerce')

        # # Replace inf and NaN with safe values (0 or column mean)
    ...
    **Note:** The Time feature is automatically excluded from predictions as transactions are independent of time.
    ...
    if app_mode == "Data Overview":
        ...
        selection_method = st.radio(
            "Selection Method:",
            ["Dropdown Select", "Manual Index Entry"],
            horizontal=True
        )

        selected_idx = None  # will hold the final row index

        
        # SELECT FROM DROPDOWN (existing behavior)
        
        if selection_method == "Dropdown Select":
        ...
        # MANUAL INDEX ENTRY
            ...
            user_input = st.text_input(f"Enter index (0 – {len(df)-1}):")
            ...
            if user_input.strip():
                if not user_input.isdigit():
                    ...
                    typed_idx = int(user_input)
                    if typed_idx < 0 or typed_idx >= len(df):
                        st.error("Index out of range. Please enter a valid index.")
                        st.stop()
                    selected_idx = typed_idx

        
        # RUN PREDICTION IF AN INDEX WAS SELECTED
            ...
            if st.button("Predict Selected Transaction"):
                ...
                if feature_dict:
                    ...
                    with st.spinner("Making prediction..."):
                        processed_input, actual_class = preprocess_user_input(
                            feature_dict, scaler, power_transformer
                        )
                        prediction = model.predict(processed_input)[0]
                        probability = model.predict_proba(processed_input)[0]
